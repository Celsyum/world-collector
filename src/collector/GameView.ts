...
{
	private static NUM_COLUMNS = 4;
	private static NUM_ROWS = 4;
	...
	private addTile(row: number, column: number, animate: boolean = true): void
	{
		...
		var tile = null;
		var type = Math.round(Math.random() * (GameView.tileImages.length - 1));
	...
	private construct(): void
	{
		...
		var contentWidth = 75 * GameView.NUM_COLUMNS;
		...
		this.Background.graphics.drawRect(0, 0, contentWidth, 75 * GameView.NUM_ROWS);
